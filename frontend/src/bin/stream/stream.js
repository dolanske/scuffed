...
export class DebugLog {
  ...
  addLog(category, msg) {
    ...
    if (this.records.length >= this.maxRecords) {
      this.records[this.maxFirstRecords + this.recordCursor] = {
        n: this.logCount,
        category: category,
        time: time,
        msg: msg
  ...
  getLog() {
    ...
    function formatRecord(l) {
      return `${l.n.toString().padEnd(5)} ${l.category} [${l.time.toPrecision(5).padEnd(5)}] ${
        l.msg
      }\n`
    }
    ...
    if (this.logCount >= this.maxFirstRecords) {
      this.records.slice(this.recordCursor).forEach((l) => (log += formatRecord(l)))
      this.records
...
export class StreamStatistics {
  ...
  createStatsContainer() {
    ...
    this.statsContainer.style.cssText =
      "opacity:0.9;position:absolute;top:5px;left:5px;background-color:#222;padding:5px;"
    ...
    var greenScale = ["#DEEDCF", "#74c67a", "#1d9a6c", "#137177", "#0a2f51"]
    ...
    this.bufferPanel = stats.addPanel(
      new Stats.Panel("Buffer", greenScale, 5000, (min, max, total, avg, stddev) => {
        this.saveGraphImages()
        return `${Math.round(avg)} ms, min: ${Math.round(min)}, max: ${Math.round(
          max
        )}, std-dev: ${stddev.toPrecision(2)} ms`
      })
    )
    this.networkPanel = stats.addPanel(
      new Stats.Panel("Network", redScale, 1024 * 8, (min, max, total, avg, stddev) => {
        return `${Math.round((total / 1024) * 8)} Kbit/s, std-dev: ${(
          (stddev / 1024) *
          8
        ).toPrecision(4)} Kbit`
      })
    ...
    this.fpsPanel = stats.addPanel(
      new Stats.Panel("Framerate", greenScale, 60, (min, max, total, avg, stddev) => {
  ...
  onFrame(frame) {
    this.frames.push({
      t: performance.now(),
      frame: frame,
      buffered: this.stream.getBufferedVideoDuration() * 1000
    })
  }
